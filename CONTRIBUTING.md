# Berkontribusi ke Zusantara Core
...
- **Laporan bug dan ide** masuk ke [Issues](https://github.com/melkimahdali/zusantara-core/issues). Untuk celah keamanan, **jangan** buka issue publik: ikuti [SECURITY.md](SECURITY.md).
...
```bash
git clone https://github.com/melkimahdali/zusantara-core.git
cd zusantara-core
npm install          # memasang kedua workspace: packages/zusantara dan packages/create-zusantara
npm run build
npm run typecheck
npm test
```
...
| `npm run e2e` | simulasi publish: `npm pack`, buat proyek dari tarball, install, test, jalankan server, CLI global, tool Zusantara AI, dan alur interaktif buat proyek |
| `npm run docs:build` / `npm run docs:serve` | bangun dan pratinjau situs dokumentasi (butuh `npm run build` lebih dulu) |
| `npm run ai-smoke` | uji Zusantara AI dengan provider sungguhan; **memakai kredit API**, jadi hanya dijalankan manual |

Test PostgreSQL dilewati kecuali `ZUSANTARA_TEST_POSTGRES_URL` diisi (di CI disediakan lewat service container). Contoh lokal:

```bash
docker run --rm -d -p 5432:5432 -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=zusantara_test postgres:16
ZUSANTARA_TEST_POSTGRES_URL=postgres://postgres:postgres@localhost:5432/zusantara_test npm test
```

Test memakai folder `ZUSANTARA_HOME` sementara dan menghapus `ZUSANTARA_LANG` (lihat `packages/zusantara/test/setup.mjs`), jadi preferensi `zusantara lang` di komputer Anda tidak memengaruhi hasil.
...
2. **Setiap bug dari pemakaian sungguhan masuk ke e2e.** Bila bug ditemukan saat memakai Zusantara (bukan hanya dari test), tambahkan skenario yang mereproduksinya ke `scripts/e2e.mjs`, bukan hanya test unit.
3. **Setiap fitur tersedia lewat CLI dan Zusantara AI, di terminal dan browser.** Perintah CLI baru juga harus bisa dijalankan AI (lihat tool `zusantara` di `packages/zusantara/src/ai/tools.ts`), dan berfungsi di CLI interaktif maupun chat browser.
...
- **Teks di kode** (CLI, halaman bawaan, pesan error, kit UI, instruksi AI) tidak ditulis langsung, tetapi lewat katalog di `packages/zusantara/src/i18n/id/` dan `packages/zusantara/src/i18n/en/`, lalu dipanggil dengan `t()`. Katalog Indonesia adalah sumber tipe (`Messages`), jadi kunci yang lupa diterjemahkan membuat `typecheck` gagal.
- **Template proyek:** `packages/create-zusantara/templates/` berbahasa Indonesia; file pengganti Bahasa Inggris ada di `packages/create-zusantara/locales/en/` dengan path yang sama.
...
- Hindari dependensi baru di paket `zusantara` kecuali benar-benar perlu; dependensi opsional (mis. `drizzle-orm`, `postgres`) dimuat hanya saat dipakai.
...
- Test yang menyentuh AI memakai provider tiruan (lihat `packages/zusantara/test/ai/`), tidak pernah API sungguhan.
- Perubahan pada aturan keamanan Zusantara AI (path terlarang, risiko tool, persetujuan, sensor rahasia) wajib disertai test yang membuktikan aturan itu tetap berlaku.
...
- **Bugs and ideas** go to [Issues](https://github.com/melkimahdali/zusantara-core/issues). For security vulnerabilities, **do not** open a public issue: follow [SECURITY.md](SECURITY.md).
...
git clone https://github.com/melkimahdali/zusantara-core.git
cd zusantara-core
npm install          # installs both workspaces: packages/zusantara and packages/create-zusantara
npm run build
npm run typecheck
npm test
```
...
| `npm run e2e` | publish simulation: `npm pack`, create a project from the tarballs, install, test, run the server, global CLI, Zusantara AI tools, and the interactive create-project flow |
| `npm run docs:build` / `npm run docs:serve` | build and preview the documentation site (run `npm run build` first) |
| `npm run ai-smoke` | test Zusantara AI against a real provider; **uses API credits**, so it only runs manually |

PostgreSQL tests are skipped unless `ZUSANTARA_TEST_POSTGRES_URL` is set (CI provides one through a service container). Locally:
...
Tests use a temporary `ZUSANTARA_HOME` and unset `ZUSANTARA_LANG` (see `packages/zusantara/test/setup.mjs`), so your own `zusantara lang` preference does not affect the results.
...
2. **Every bug found in real use goes into e2e.** If a bug shows up while using Zusantara (not just in tests), add a scenario that reproduces it to `scripts/e2e.mjs`, not only a unit test.
3. **Every feature is available through the CLI and Zusantara AI, in the terminal and the browser.** A new CLI command must also be runnable by the AI (see the `zusantara` tool in `packages/zusantara/src/ai/tools.ts`), and work in the interactive CLI and the browser chat.
...
- **Text in code** (CLI, built-in pages, error messages, UI kit, AI instructions) is never written inline; it lives in the catalogs under `packages/zusantara/src/i18n/id/` and `packages/zusantara/src/i18n/en/` and is read with `t()`. The Indonesian catalog is the source of the `Messages` type, so a missing translation fails `typecheck`.
- **Project templates:** `packages/create-zusantara/templates/` is Indonesian; English replacement files live in `packages/create-zusantara/locales/en/` under the same paths.
...
- Avoid new dependencies in the `zusantara` package unless truly needed; optional dependencies (e.g. `drizzle-orm`, `postgres`) are loaded only when used.
...
- Tests that involve the AI use fake providers (see `packages/zusantara/test/ai/`), never a real API.
- Changes to Zusantara AI's safety rules (forbidden paths, tool risk levels, approvals, secret redaction) must come with tests proving the rule still holds.
