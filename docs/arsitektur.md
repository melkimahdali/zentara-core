...
order: 6
group: Referensi
description: Peta kode Zusantara Core untuk kontributor: paket, runtime, CLI, Zusantara AI, dan alur pengembangan.
---

# Arsitektur

Halaman ini untuk Anda yang ingin memahami atau mengubah kode Zusantara Core. Untuk memakai framework, mulai dari [Mulai cepat](mulai-cepat.html). Kontrak perilaku yang dijaga (dan dianggap *breaking change* bila berubah) ada di [`CORE_SPEC.md`](https://github.com/melkimahdali/zusantara-core/blob/main/CORE_SPEC.md); cara menyiapkan repo ada di [`CONTRIBUTING.md`](https://github.com/melkimahdali/zusantara-core/blob/main/CONTRIBUTING.md).

## Isi repo

| Path | Isi |
|---|---|
| `packages/zusantara` | paket `zusantara`: runtime framework, CLI `zusantara`, Zusantara AI, kit UI, back-end |
| `packages/create-zusantara` | `npm create zusantara`: menyalin template `api` atau `minimal` |
...
Isi `packages/zusantara/src/` dibagi per lapisan. Titik masuk publik diatur `exports` di `package.json`:

| Import | Berkas | Isi |
|---|---|---|
| `zusantara` | `core/index.ts` | runtime, middleware, session, validasi, view, auth, back-end |
| `zusantara/ui` | `ui/index.ts` | kit UI server-side dan halaman bawaan |
| `zusantara/db` | `db/index.ts` | `createSqlite`, `createPostgres`, migrasi (butuh `drizzle-orm`, opsional) |
| `zusantara/host` | `repl/host.ts` | inti CLI interaktif untuk tampilan lain (`HostUI`) |
| bin `zusantara` | `cli.ts` | CLI |

| Folder | Tanggung jawab |
|---|---|
| `core/` | runtime HTTP: config, router, context, middleware, respons, error, cookie, session, CSRF/CORS, rate limit, validasi, view, file statis, auth, aset bawaan `/_zusantara/*` |
| `core/devpage/` | halaman sambutan, halaman error lengkap, dan widget chat Zusantara AI untuk browser |
...
| `ui/` | kit UI (`zusantara/ui`), gaya, dan font brand |
| `ai/` | Zusantara AI: loop agen, tool, persetujuan, jurnal undo, provider, OmniRoute, sesi tersimpan |
...
| `serve.ts` | menjalankan aplikasi pengguna (dipakai `zusantara dev` dan `zusantara start`) |
...
1. **`new ZenRuntime(config)`**: `resolveConfig()` (`core/config.ts`) menggabungkan `zusantara.config.mjs` dengan env lalu memvalidasinya. Bahasa aplikasi diatur dari `locale`.
  ...
  └─ createContext()                     ctx: req/res, params, query, cookies, body()
      └─ middleware global               config.middleware → plugin (runtime.use) → src/app/middleware.ts
          └─ route()
              ...
              ├─ /_zusantara/*             aset bawaan (CSS kit UI, font, logo)
...
- **Perintah biasa** (`dev`, `build`, `start`, `routes`, `make:*`, `db:*`, `jobs*`, `lang`, `undo`, `ai:*`) dijalankan langsung. Perintah database di CLI global diteruskan ke CLI `zusantara` milik proyek (`node_modules/zusantara`), karena `drizzle-orm` ada di sana (`process.ts`, `findLocalCli`).
- **Teks bebas** (argumen berspasi, atau `zusantara ai "..."`) dijalankan sebagai tugas Zusantara AI sekali jalan di terminal.
...
tugas (terminal / browser)
  └─ ai/session.ts       satu percakapan, simpan ke .zusantara/sessions/
      └─ ai/agent.ts     loop: panggil model → jalankan tool → ulangi (maks. ai.maxSteps)
          ...
          └─ ai/journal.ts      catat isi file sebelum diubah → zusantara undo
...
- **Pagar pengaman** dipusatkan di `ai/tools.ts` (path terlarang, `.env`, file database, symlink) dan `ai/approval.ts` (kebijakan persetujuan). Aturan persisnya di `CORE_SPEC.md` bagian Zusantara AI, dan setiap perubahan di sini harus disertai test.
- **Konfigurasi dan provider** di `ai/config.ts`, `ai/presets.ts`, dan `ai/setup.ts` (`zusantara ai:setup`). Percakapan disimpan dalam format netral, jadi rantai bisa berpindah provider di tengah sesi.

### Chat di browser

Saat `zusantara dev` atau CLI interaktif berjalan, `dev/devtools.ts` membuka server kecil di `127.0.0.1` dengan token acak per sesi. Port dan token diteruskan ke proses aplikasi lewat `ZUSANTARA_DEVTOOLS_PORT` dan `ZUSANTARA_DEVTOOLS_TOKEN`; halaman sambutan dan halaman error (`core/devpage/`) lalu menyisipkan widget chat (`core/devpage/chat.ts`) yang berbicara dengan server itu (`/status`, `/chat`, `/approve`, `/stop`, `/undo`, `/reset`). Server devtools memakai sesi AI dan aturan yang sama dengan terminal, dan kunci bersama (`AiLock`) mencegah tugas terminal dan browser berjalan bersamaan.
...
zusantara dev
  ...
  └─ tsx watch serve.ts               memantau src/app, .env, zusantara.config.mjs → restart otomatis
        env: NODE_ENV=development, ZUSANTARA_APP_DIR=src/app, ZUSANTARA_DEVTOOLS_*
        └─ ZenRuntime                 aplikasi pengguna
```

`zusantara build` menjalankan `tsc` milik proyek, dan `zusantara start` menjalankan `serve.ts` langsung dari `dist/app` dengan `NODE_ENV=production`.

## Bahasa

`i18n/index.ts` menentukan bahasa: env `ZUSANTARA_LANG`, lalu `locale` di config proyek, lalu `~/.zusantara/settings.json` (`zusantara lang`), lalu Bahasa Indonesia. Semua teks antarmuka diambil lewat `t()` dari katalog `i18n/id/` (sumber tipe `Messages`) dan `i18n/en/`. Di `create-zusantara`, template Indonesia ada di `templates/` dan file pengganti Bahasa Inggris di `locales/en/`.
...
| AI dengan provider tiruan | `packages/zusantara/test/ai/` | `npm test` |
