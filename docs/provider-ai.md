...
Zusantara bisa memasang dan menjalankan OmniRoute sendiri. Tidak perlu membuka terminal lain.

1. **Pasang.** Pilih salah satu:
   - saat `npm create zusantara@latest`, jawab **Y** pada *"Pasang OmniRoute sekarang?"*;
   - di CLI interaktif (`npx zusantara`), pilih **Ya, pasang & jalankan** saat ditawari, atau ketik `/omniroute install`;
   - lewat wizard `npx zusantara ai:setup omniroute`;
   - atau manual: `npm install -g omniroute`.

   OmniRoute butuh Node.js 22.22+ atau 24+, dan cukup dipasang sekali untuk semua proyek.
2. **Jalankan.** `npx zusantara` otomatis menawarkan menjalankan OmniRoute di latar belakang dan mematikannya lagi saat Anda keluar. Perintah lain:
...
4. **(Opsional) Tambah provider gratis** di dashboard OmniRoute http://localhost:20128, menu **Providers**, mis. *OpenCode Free* (tanpa login) atau *Kiro*. Bila dashboard meminta API key untuk endpoint, salin dari **Endpoints** lalu simpan dengan `npx zusantara ai:setup omniroute`.

Bila OmniRoute tidak berjalan, Zusantara otomatis memakai provider berikutnya di rantai (mis. OpenAI atau Claude bila API key-nya diisi).

Untuk memilih provider lain:

```bash
npx zusantara ai:setup            # menu: pilih provider, ketik API key (tersembunyi), pilih model, tes koneksi
npx zusantara ai:setup openai     # langsung ke provider tertentu
npx zusantara ai:status           # lihat rantai provider yang aktif
```

Hasilnya disimpan ke `.env` (izin 0600, tidak ikut ter-commit), bukan ke file config.

Zusantara mencoba provider **berurutan**. Kalau satu provider kehabisan kredit (402), kena batas kuota (429), mati, atau belum diatur, Zusantara otomatis pindah ke provider berikutnya tanpa kehilangan percakapan. Provider cloud **otomatis ikut** begitu API key-nya ada di `.env`:
...
| Claude | `ANTHROPIC_API_KEY` | `ZUSANTARA_CLAUDE_MODEL` (`claude-opus-5`) | selalu ada di rantai; dilewati bila key kosong |
...
Pengaturan lain:
- **Urutan:** default OmniRoute → Claude → provider cloud yang key-nya terisi → Ollama. Ubah dengan `ZUSANTARA_AI_ORDER=openai,omniroute,claude` (provider lain yang aktif menyusul di belakang), atau jawab "Ya" pada "Jadikan provider utama?" di `ai:setup`.
...
Untuk kendali penuh, tulis rantainya sendiri di `zusantara.config.mjs`. Bila diisi, pengaturan dari `.env` di atas tidak dipakai:
