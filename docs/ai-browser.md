---
title: Zusantara AI di browser
order: 3
group: Zusantara AI
description: Halaman sambutan, halaman error, chat AI di setiap halaman, dan AI yang bisa melihat tampilan.
---

# Zusantara AI di browser

Selama pengembangan (`npx zusantara` atau `npm run dev`), Zusantara AI juga bisa dipakai dari browser:

- **Halaman sambutan** (`src/app/routes/index.ts` bawaan template) menampilkan status aplikasi, daftar route, dan kotak chat Zusantara AI. Ganti file itu untuk halaman Anda sendiri; pakai lagi kapan saja dengan `export { welcomePage as GET } from "zusantara";`.
- **Halaman error** menampilkan pesan, stack trace dengan potongan kode yang disorot, penyebab (`cause`), dan detail request. Tombol **✦ Tanya Zusantara AI** mengirim error itu ke AI, yang menjelaskan penyebabnya lalu mengusulkan perbaikan. Setiap perubahan tetap meminta persetujuan (diff + tombol Setujui/Tolak) dan bisa dibatalkan.
...
Selama pengembangan, setiap halaman aplikasi Anda (bukan hanya halaman sambutan dan error) mendapat tombol **Tanya Zusantara AI** yang mengambang di pojok kanan bawah. Fungsinya sama dengan chat di halaman error: minta perubahan, lihat diff lalu Setujui/Tolak, Batalkan perubahan, Berhenti, dan Percakapan baru. Percakapan tetap ada setelah halaman dimuat ulang.
...
Setelah mengubah halaman, Zusantara AI **wajib** memanggil tool `view_page` untuk halaman itu di layar desktop dan ponsel, lalu memperbaiki temuannya, sama seperti typecheck dan test. Bila setelah dua percobaan masih ada masalah, AI melaporkan temuannya apa adanya dan tugas tidak ditandai selesai.
...
Varian `theme` dan `lang` hanya berlaku untuk request itu (lewat parameter `__zusantara_mode` dan `__zusantara_lang` yang dibuang sebelum routing), jadi halaman lain dan tab Anda tidak berubah.
...
`screenshot: true` (atau `zusantara view --screenshot`) membuka halaman di Chrome, Chromium, atau Edge headless yang sudah terpasang di komputer Anda, dengan ukuran layar yang sama, lalu menyimpannya di `.zusantara/screenshots/`. Tidak ada browser yang diunduh; bila tidak ditemukan, isi `CHROME_PATH` dengan lokasi file browser. Halaman dibuka tanpa login dan tanpa widget pengembangan.
...
| `kit` | halaman tidak dibuat dengan `page()` dari `zusantara/ui` | ✓ | ✓ |
...
Hasil yang sama bisa Anda lihat sendiri. Bila `zusantara dev` atau CLI interaktif berjalan dan ada tab browser yang terbuka, `zusantara view` memakai tab itu; bila tidak, versi teks. Kode keluarnya 1 bila ada temuan, error, atau teks `--text` yang tidak ada.

```bash
npx zusantara view /notes                    # elemen, error console, pemeriksaan tampilan, dan skor
npx zusantara view /notes --mobile           # layar ponsel (390 px); --tablet untuk 768 px
npx zusantara view /notes --dark --lang en   # varian mode gelap dan Bahasa Inggris
npx zusantara view /notes --screenshot       # simpan gambar PNG di .zusantara/screenshots/
npx zusantara view /login --text "Masuk"     # gagal bila teks tidak ada
npx zusantara view / --min-score 90          # gagal bila skor di bawah 90
npx zusantara view /api/hello --json
```

## Alat pengembang

Semua alat ini hanya ada selama `zusantara dev` berjalan, dan semua datanya juga bisa dibaca Zusantara AI.

### Toolbar request

Di samping tombol **Tanya Zusantara AI** ada tombol kecil berisi waktu proses request halaman itu dan jumlah query, mis. `42 ms · 3 query`. Tanda merah **N+1** muncul bila query yang sama dijalankan tiga kali atau lebih dalam satu request (biasanya query di dalam perulangan; ambil sekaligus dengan join atau `inArray`). Klik untuk melihat:
...
```bash
npx zusantara requests                 # 50 request terakhir, terbaru di atas
npx zusantara requests --path /notes   # hanya path yang diawali /notes
npx zusantara requests <id>            # rincian: query, N+1, session, log
npx zusantara requests --json
```

Zusantara AI memakai tool `request_log` untuk hal yang sama, dan hasil `view_page` sudah menyertakan request halaman yang dilihat. Setiap respons membawa header `X-Zusantara-Request` berisi id-nya.
...
Setelah file berubah dan server dev selesai mulai ulang, semua tab aplikasi yang terbuka dimuat ulang. Tab yang punya isian formulir belum terkirim tidak dimuat ulang (muncul pemberitahuan), dan selama Zusantara AI mengerjakan tugas dari browser, muat ulang ditunda sampai tugas selesai.

### Input suara

Tombol mikrofon di kotak chat (halaman sambutan, halaman error, dan widget) mengubah ucapan jadi teks dalam bahasa Zusantara (`id-ID` atau `en-US`). Tombol ini hanya muncul di browser yang mendukung Web Speech API (Chrome, Edge, Safari). Di Chrome, pengenalan suara diproses oleh layanan Google.

## Journal hasil tugas AI

Setiap tugas Zusantara AI (terminal, CLI interaktif, dan chat di browser) mencatat ringkasannya ke `.zusantara/ai-tasks.jsonl`: status, jumlah langkah, durasi, token, hasil typecheck dan test, serta setiap `view_page`. Isi file dan percakapan tidak dicatat. Hanya baris pertama permintaan yang disimpan, dan journal tidak pernah dikirim keluar dari komputer Anda. Eval AI di Tahap 15 memakai data ini.

```bash
npx zusantara ai:log               # 20 tugas terakhir dan persentase yang selesai
...
- server aplikasi hanya menyisipkannya bila mode debug aktif, aplikasi dijalankan oleh `zusantara dev` atau CLI interaktif (yang mengisi `ZUSANTARA_DEV=1` dan token devtools), dan bukan `NODE_ENV=production`;
- `zusantara start` menghapus variabel server pengembangan dari env;
- di produksi `/_zusantara/dev/probe.js`, `/_zusantara/dev/widget.js`, dan `/_zusantara/dev/requests` menjawab 404, HTML Anda tidak diubah (tanpa `data-zsrc`), jejak request tidak dicatat, dan parameter varian `__zusantara_*` tidak berlaku;
- potongan HTML (tanpa `<html>`/`<body>`) dan request htmx tidak pernah disisipi.

Uji e2e memastikan halaman produksi tidak memuat widget, termasuk saat env devtools sengaja terbawa dan `ZUSANTARA_DEBUG=1`.
...
- `zusantara dev` menulis port dan token devtools ke `.zusantara/devtools.json` (hanya bisa dibaca pemilik file, dihapus saat server berhenti) supaya `zusantara view` dari terminal lain bisa memakai tab browser;
...
Di produksi (`zusantara start`), pengunjung hanya melihat halaman error sederhana tanpa detail, dan halaman sambutan tanpa chat maupun daftar route. Klien API (`Accept: application/json`) tetap mendapat JSON.
