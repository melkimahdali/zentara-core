...
order: 2
group: Memulai
description: Chat dengan Zusantara AI di terminal, gaya Claude Code.
---

# CLI interaktif

Pasang sekali secara global agar cukup mengetik `zusantara` dari folder mana pun, seperti Claude Code:
...
(Tanpa pemasangan global: `npx zusantara` di folder proyek.)
...
Saat dibuka, `zusantara` pindah ke layar alternatif terminal (seperti vim atau htop), jadi output sebelumnya, mis. `npm install`, tidak terlihat dan tidak bisa digulir. Judul tab terminal menjadi *zusantara*. Layar dibagi tiga:

1. **Header berbingkai terkunci di atas:** logo Z kecil, nama dan versi Zusantara Core, tagline, AI dan mode yang aktif, dan folder proyek; di terminal lebar juga tips perintah dan status server dev. Di terminal pendek (kurang dari 24 baris) header diringkas menjadi dua baris. Header tidak ikut bergulir.
...
Atau untuk sementara: `ZUSANTARA_FULLSCREEN=off zusantara`.
...
`zusantara --classic` (atau `ZUSANTARA_UI=classic`) memakai CLI klasik tanpa Ink.

Saat dibuka, logo Zusantara Core muncul dengan animasi singkat (±1 detik): tersapu mengikuti goresan Z dengan kilau Pearl, lalu motif emas menyusul. Animasi ini otomatis mati di CI. Untuk mematikannya:
...
Atau untuk sementara: `ZUSANTARA_ANIMATION=off zusantara` (PowerShell: `$env:ZUSANTARA_ANIMATION="off"; zusantara`).
...
- **Di luar folder proyek:** muncul pilihan *Buat proyek baru* (nama folder dan template ditanyakan di dalam Zusantara, proyek dibuat dan `npm install` berjalan di latar belakang dengan progres di spinner, lalu Zusantara langsung terbuka di proyek itu; **Esc** membatalkan), *Chat di folder ini*, atau *Buka dokumentasi*.

Fitur sesi interaktif:

- **Server dev di latar belakang.** Saat dibuka, Zusantara bertanya dulu *"Jalankan server dev (npm run dev) di latar belakang?"*. Bila Ya, tidak perlu membuka terminal kedua; lognya disimpan (lihat dengan `/logs`) dan error server muncul di baris status. AI juga bisa membaca log itu untuk mencari penyebab error, dan menyalakan server hanya setelah Anda setujui. Bila `npm run dev` sudah berjalan di terminal lain, Zusantara memakainya. Lewati pertanyaannya dengan `--no-dev`.
- **Jawaban mengalir (streaming).** Teks AI muncul baris demi baris selagi ditulis, tidak perlu menunggu jawaban selesai.
- **Percakapan berlanjut dan tersimpan.** Permintaan berikutnya bisa merujuk yang sebelumnya ("ubah warnanya jadi biru"). Percakapan disimpan otomatis di `.zusantara/sessions/` (30 terbaru, diabaikan git), jadi setelah menutup terminal Anda bisa melanjutkannya dengan `/resume` atau `zusantara --continue`.
...
- **Perintah terminal.** AI bisa menjalankan perintah seperti `git diff` atau `npx eslint src`. Lihat [aturan keamanannya](zusantara-ai.html#perintah-terminal).
