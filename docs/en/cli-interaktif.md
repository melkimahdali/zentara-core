...
description: Chat with Zusantara AI in the terminal, Claude Code style.
---

# Interactive CLI

Install it globally once so you can type `zusantara` from any folder, just like Claude Code:
...
(Without a global install: `npx zusantara` in the project folder.)
...
When it opens, `zusantara` switches to the terminal's alternate screen (like vim or htop), so earlier output such as `npm install` is hidden and can't be scrolled to. The terminal tab title becomes *zusantara*. The screen has three sections:

1. **A framed header pinned at the top:** a small Z logo, the Zusantara Core name and version, the tagline, the active AI and mode, and the project folder; on wide terminals also command tips and the dev server status. On short terminals (fewer than 24 rows) the header shrinks to two lines. The header does not scroll.
...
Or just once: `ZUSANTARA_FULLSCREEN=off zusantara`.
...
`zusantara --classic` (or `ZUSANTARA_UI=classic`) uses the classic CLI without Ink.

On start, the Zusantara Core logo plays a short animation (about 1 second): it sweeps along the Z stroke with a Pearl shimmer, then the gold motif follows. The animation is off automatically in CI. To turn it off:
...
Or just once: `ZUSANTARA_ANIMATION=off zusantara` (PowerShell: `$env:ZUSANTARA_ANIMATION="off"; zusantara`).
...
- **Outside a project folder:** you can choose *Create a new project* (the folder name and template are asked inside Zusantara, the project is created and `npm install` runs in the background with progress in the spinner, then Zusantara opens in that project right away; **Esc** cancels), *Chat in this folder*, or *Open the docs*.

Interactive session features:

- **Dev server in the background.** On start, Zusantara asks *"Start the dev server (npm run dev) in the background?"*. If you say yes, there's no need for a second terminal; its log is kept (see it with `/logs`) and server errors show up in the status line. The AI can also read that log to find the cause of an error, and only starts the server after you approve. If `npm run dev` is already running in another terminal, Zusantara uses it. Skip the question with `--no-dev`.
- **Streaming responses.** AI text appears line by line as it is written; no waiting for the full answer.
- **Ongoing, saved conversations.** Follow-up requests can refer to earlier ones ("make it blue"). Conversations are saved automatically in `.zusantara/sessions/` (the latest 30, ignored by git), so after closing the terminal you can continue with `/resume` or `zusantara --continue`.
...
- **Terminal commands.** The AI can run commands such as `git diff` or `npx eslint src`. See the [safety rules](zusantara-ai.html#terminal-commands).
- **Slash commands:**
