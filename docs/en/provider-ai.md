...
Zentara can install and run OmniRoute for you. No need to open another terminal.

1. **Install.** Pick one:
   - during `npm create zentara@latest`, answer **Y** to *"Install OmniRoute now?"*;
   - in the interactive CLI (`npx zentara`), accept the offer to install it, or type `/omniroute install`;
   - through the wizard: `npx zentara ai:setup omniroute`;
   - or manually: `npm install -g omniroute`.

   OmniRoute needs Node.js 22.22+ or 24+, and you only install it once for all projects.
2. **Run.** `npx zentara` offers to start OmniRoute in the background and stops it again when you exit. Other commands:
...
4. **(Optional) Add free providers** in the OmniRoute dashboard at http://localhost:20128, under **Providers**, e.g. *OpenCode Free* (no sign-in) or *Kiro*. If the dashboard asks for an endpoint API key, copy it from **Endpoints** and save it with `npx zentara ai:setup omniroute`.

If OmniRoute isn't running, Zentara automatically uses the next provider in the chain (e.g. OpenAI or Claude if their API keys are set).

To choose another provider:

```bash
npx zentara ai:setup            # menu: pick a provider, type the API key (hidden), pick a model, test the connection
npx zentara ai:setup openai     # go straight to one provider
npx zentara ai:status           # show the active provider chain
```

The result is saved to `.env` (mode 0600, never committed), not to the config file.

Zentara tries providers **in order**. If one runs out of credit (402), hits a quota limit (429), is down, or isn't configured, Zentara moves on to the next one without losing the conversation. Cloud providers **join automatically** as soon as their API key is in `.env`:
...
| Claude | `ANTHROPIC_API_KEY` | `ZENTARA_CLAUDE_MODEL` (`claude-opus-5`) | always in the chain; skipped if the key is empty |
...
Other settings:
- **Order:** by default OmniRoute → Claude → cloud providers with a key → Ollama. Change it with `ZENTARA_AI_ORDER=openai,omniroute,claude` (other active providers follow), or answer "Yes" to "Make it the primary provider?" in `ai:setup`.
...
For full control, write the chain yourself in `zentara.config.mjs`. When set, the `.env` settings above are not used:
