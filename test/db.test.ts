...
function sqliteMigrations(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-mig-"));
...
describe("SQLite (node:sqlite)", () => {
  it("migrasi, CRUD, relational query, dan file dibuat otomatis", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-db-"));
...
const PG_URL = process.env.ZENTARA_TEST_POSTGRES_URL;
describe("PostgreSQL", { skip: PG_URL ? false : "set ZENTARA_TEST_POSTGRES_URL untuk menjalankan" }, () => {
  it("migrasi, CRUD, dan transaksi", async () => {
    const db = await createPostgres(PG_URL!, pgSchema);
    try {
      assert.equal(databaseDialect(db), "postgres");
      await db.execute(sql`drop table if exists zentara_test_items; drop schema if exists drizzle cascade`);
