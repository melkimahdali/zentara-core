...
/**
 * Jejak request saat pengembangan (`zentara requests`, tool `request_log`, dan bagian "Request" di hasil
 * `view_page`): waktu proses, query database beserta waktunya, query berulang (N+1), session, dan log.
 * Jejaknya dicatat server aplikasi (lihat core/devtrace.ts) dan dibaca lewat /_zentara/dev/requests
...
export async function fetchTraces(appUrl: string, token: string, id?: string, signal?: AbortSignal): Promise<RequestTrace[]> {
  const url = new URL(`/_zentara/dev/requests${id ? `/${encodeURIComponent(id)}` : ""}`, appUrl);
  const res = await fetch(url, { headers: { "X-Zentara-Token": token, Accept: "application/json" }, signal: AbortSignal.any([AbortSignal.timeout(5000), ...(signal ? [signal] : [])]) });
