...
/**
 * CLI interaktif klasik (`zentara --classic`): tampilan readline biasa di atas inti yang sama dengan
...
export async function startRepl(options: ReplOptions): Promise<number> {
  ...
  /** Keluaran AI (Markdown mengalir, tool) memakai tampilan terminal yang sama dengan `zentara "..."`. */
...
/** Header gaya Claude Code: logo Zentara Core di kiri, versi, AI, dan folder di kanan. */
function printBanner(io: Output, info: { version: string; shortCwd: string }, status: HostStatus): void {
  ...
  const aiLine = status.provider
    ...
    : c.yellow(m.aiNotSet);
  const lines = [`${c.bold("Zentara")} ${c.bold(accent("Core"))} ${c.dim(`v${info.version}`)}`, aiLine, c.dim(info.shortCwd)];
