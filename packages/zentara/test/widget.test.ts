...
describe("widget chat: hanya disisipkan oleh server pengembangan", () => {
  ...
  it("disisipkan bila debug + ZENTARA_DEV + devtools", () => {
    devApp(true);
    restore = setEnv({ ...DEV_ENV, NODE_ENV: undefined });
    const out = injectDevTools(DOC, { route: "src/app/routes/notes/index.ts" });
    assert.match(out, /<head><script src="\/_zentara\/dev\/probe\.js" data-port="4567" data-route="src\/app\/routes\/notes\/index\.ts"><\/script><title>/);
    assert.match(out, /<script src="\/_zentara\/dev\/widget\.js" data-port="4567" data-token="tok" data-ui="on" defer><\/script><\/body>/);
  });

  for (const [name, env, debug] of [
    ["tanpa ZENTARA_DEV", { ...DEV_ENV, ZENTARA_DEV: undefined }, true],
    ["NODE_ENV=production", { ...DEV_ENV, NODE_ENV: "production" }, true],
    ["mode debug mati", DEV_ENV, false],
    ["tanpa token devtools", { ...DEV_ENV, ZENTARA_DEVTOOLS_TOKEN: undefined }, true],
  ] as const) {
  ...
  it("potongan HTML, request htmx, dan halaman yang sudah punya chat", () => {
    ...
    const own = injectDevTools(`<!doctype html><html><head></head><body><script>window.ZentaraChat = {}</script></body></html>`);
...
describe("widget chat di server aplikasi", () => {
  ...
  before(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-widget-"));
  ...
  it("saat pengembangan: halaman memuat widget, script tersedia, JSON dan potongan tidak diubah", async () => {
    ...
    try {
      ...
      assert.match(page, /\/_zentara\/dev\/probe\.js/);
      assert.match(page, /\/_zentara\/dev\/widget\.js/);
      assert.match(page, /data-route="[^"]*routes\/index\.ts"/);
      const widget = await fetch(`${base}/_zentara/dev/widget.js`);
      assert.equal(widget.status, 200);
      assert.match(widget.headers.get("content-type") ?? "", /javascript/);
      const js = await widget.text();
      assert.match(js, /window\.ZentaraChat = /);
      assert.match(js, /Tanya Zentara AI/);
      assert.match(await (await fetch(`${base}/_zentara/dev/probe.js`)).text(), /__zentaraDev/);
      ...
      assert.match(notFound, /\/_zentara\/dev\/widget\.js/);
    } finally {
      await runtime.stop();
    }
  });
  ...
  it("produksi: HTML tanpa widget dan script 404, walau env devtools terbawa", async () => {
    ...
    try {
      ...
      assert.doesNotMatch(page, /_zentara\/dev/);
      assert.equal((await fetch(`${base}/_zentara/dev/widget.js`)).status, 404);
      assert.equal((await fetch(`${base}/_zentara/dev/probe.js`)).status, 404);
    } finally {
      await runtime.stop();
    }
  });
...
describe("devtools: kanal halaman untuk view_page", () => {
  ...
  const auth = () => ({ "X-Zentara-Token": dt.token, "Content-Type": "application/json" });

  it("env server aplikasi memuat ZENTARA_DEV", () => {
    assert.equal(dt.env.ZENTARA_DEV, "1");
  });
...
describe("skrip proyek untuk verifikasi AI", () => {
  it("tidak mewarisi PORT server dev (test tidak bentrok dengan server yang berjalan)", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-script-"));
    fs.writeFileSync(path.join(dir, "package.json"), JSON.stringify({ name: "uji", scripts: { test: 'node -e "process.stdout.write(String(process.env.PORT) + String(process.env.ZENTARA_DEV))"' } }));
    const restore = setEnv({ PORT: "4499", ZENTARA_DEV: "1" });
    try {
