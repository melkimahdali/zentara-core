...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-layout-"));
  write("package.json", JSON.stringify({ name: "demo" }));
});
afterEach(() => fs.rmSync(root, { recursive: true, force: true }));

describe("layout proyek untuk Zentara AI", () => {
  ...
  it("tanpa layout: arahkan ke page() dari zentara/ui; zenstyles lama ditandai", () => {
    write("zenstyles/app.zs.css", "body{}");
    const lines = layoutSnapshot(root);
    assert.match(lines[0]!, /none yet.*page\(\).*zentara\/ui/);
