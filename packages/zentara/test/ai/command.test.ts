...
describe("classifyCommand", () => {
  ...
  it("menolak admin/sistem, shell bersarang, kredensial, rahasia, path luar, dan server", () => {
    const rejected = [
      ...
      "npm run dev", "npx zentara dev", "npx tsc --watch", "./node_modules/.bin/tsc",
    ];
...
describe("redaksi rahasia", () => {
  let root: string;
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-cmd-"));
  });
...
describe("tool run_command", () => {
  ...
  it("output yang memuat rahasia disensor sebelum dikirim ke AI", async () => {
    process.env.ZENTARA_TEST_TOKEN = "token-uji-rahasia";
    try {
      const out = await tool.run({ command: "node --print process.env.ZENTARA_TEST_TOKEN" }, ctx("auto"));
      ...
      delete process.env.ZENTARA_TEST_TOKEN;
    }
  });
