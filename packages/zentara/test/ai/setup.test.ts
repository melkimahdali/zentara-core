...
describe("provider dari .env", () => {
  ...
  it("ZENTARA_AI_ORDER, model & alamat dari env", () => {
  ...
  it("provider tak dikenal di ZENTARA_AI_ORDER -> error jelas", () => {
    assert.throws(() => defaultProviders({ ZENTARA_AI_ORDER: "gpt" }), /tidak dikenal "gpt"/);
  });
...
describe("helper ai:setup", () => {
  it("upsertEnvFile memperbarui/menambah tanpa mengubah baris lain", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-env-"));
  ...
  it("isEnvIgnored", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-gi-"));
