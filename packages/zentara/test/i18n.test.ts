...
describe("i18n (Bahasa Indonesia & English)", () => {
  ...
  it("urutan: env ZENTARA_LANG > config > preferensi global > Indonesia", () => {
    ...
    assert.equal(resolveLocale({ env: { ZENTARA_LANG: "en" }, config: "id" }), "en");
  });

  it("preferensi global disimpan di ZENTARA_HOME/settings.json tanpa menghapus isi lain", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-set-"));
    const env = { ZENTARA_HOME: home };
  ...
  it("`zentara lang en` menyimpan preferensi; CLI lalu berbahasa Inggris", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "zentara-lang-"));
    const out: string[] = [];
    const io = { cwd, out: (l: string) => out.push(l), err: (l: string) => out.push(l) };
    assert.equal(await run(["lang"], io), 0);
    assert.match(out.join("\n"), /Bahasa Zentara: Bahasa Indonesia \(bawaan\)/);
    ...
    fs.writeFileSync(path.join(cwd, "zentara.config.mjs"), 'export default { locale: "id" };\n');
    out.length = 0;
    await run(["help"], io);
  ...
  it("config `locale` dipakai runtime; produksi mengabaikan preferensi global", () => {
    ...
    const home = { ZENTARA_HOME: process.env.ZENTARA_HOME };
  ...
  it("halaman bawaan, kit UI, dan perintah CLI mengikuti bahasa aktif", () => {
    ...
    assert.match(welcome, /zentara-core\.morixa\.id\/en\//);
    const html = page(
      { title: "t" },
    ...
    assert.equal(docsUrl("ui.html"), "https://zentara-core.morixa.id/en/ui.html");

    setLocale("id");
    assert.match(renderStatusPage(404), /Halaman tidak ditemukan/);
    assert.match(page({ title: "t" }), /Lewati ke konten/);
    assert.equal(formatNumber(12500), "12.500");
    assert.equal(docsUrl("ui.html"), "https://zentara-core.morixa.id/ui.html");
  });
});
