...
describe("create-zentara", () => {
  let tmp: string;
  before(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "create-zentara-"));
  });
  ...
  for (const template of Object.keys(TEMPLATES) as (keyof typeof TEMPLATES)[]) {
    it(`template ${template}: file lengkap, .gitignore, package.json, .env`, () => {
      ...
      assert.equal(pkg.dependencies.zentara, `^${ownVersion()}`);
      assert.equal(pkg.scripts.dev, "zentara dev");
  ...
  it("zentaraSpec bisa diganti (untuk uji paket lokal)", () => {
    scaffold({ targetDir: path.join(tmp, "spec"), template: "minimal", zentaraSpec: "file:../zentara.tgz" });
    assert.equal(JSON.parse(fs.readFileSync(path.join(tmp, "spec", "package.json"), "utf8")).dependencies.zentara, "file:../zentara.tgz");
  });

  it("helper nama, argumen, dan package manager", () => {
    assert.equal(toPackageName("  My App!! "), "my-app");
    assert.equal(toPackageName("..."), "zentara-app");
