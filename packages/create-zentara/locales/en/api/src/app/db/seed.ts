...
// Initial data: `zentara db:seed`. Safe to run repeatedly (existing data is not duplicated).
export default async function seed(db: typeof Database): Promise<string> {
  ...
  await db.insert(notes).values([
    ...
    },
    {
      userId: admin!.id,
      title: "Next idea",
      body: 'Try asking Zentara AI: npx zentara "build a booking schedule page for signed-in users".',
    },
  ]);
