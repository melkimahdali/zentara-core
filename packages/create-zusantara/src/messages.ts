...
/**
 * Bahasa bawaan: env ZUSANTARA_LANG, lalu preferensi global `zusantara lang` (~/.zusantara/settings.json),
 * lalu Bahasa Indonesia. Sama dengan urutan di paket zusantara.
 */
export function defaultLang(env: NodeJS.ProcessEnv = process.env): Lang {
  const fromEnv = parseLang(env.ZUSANTARA_LANG);
  if (fromEnv) return fromEnv;
  try {
    const dir = env.ZUSANTARA_HOME ? path.resolve(env.ZUSANTARA_HOME) : path.join(os.homedir(), ".zusantara");
...
const id = {
  ...
  help: (templates: string) => `Buat proyek Zusantara baru

  npm create zusantara@latest [folder] [-- --template api|minimal] [--lang id|en] [--no-install] [--yes]

Template:
${templates}

Bahasa (--lang): id (Bahasa Indonesia) atau en (English). Mengatur teks aplikasi contoh, README,
dan bahasa Zusantara di proyek ini.
`,
  ...
  omnirouteIntro: (name: string, free: string) => `\nZusantara AI memakai ${name} sebagai provider default: ${free}, tanpa API key.`,
  free: "gratis",
  ...
  omnirouteOffer: "npx zusantara akan menawarkan menjalankannya di latar belakang.",
  ...
  nextSteps: "\nLangkah berikutnya:\n",
  omnirouteHint: "# AI gratis (sekali saja); atau pilih provider lain: npx zusantara ai:setup",
  zusantaraHint: "# chat dengan Zusantara AI + server dev di latar belakang",
  serverOnly: (cmd: string) => `\nAtau jalankan server saja: ${cmd} dev`,
  globalTip: 'Tip: npm install -g zusantara agar cukup mengetik "zusantara" dari folder mana pun.\n',
};
...
const en: Messages = {
  ...
  help: (templates) => `Create a new Zusantara project
...
and Zusantara's language in this project.
`,
  ...
  omnirouteIntro: (name, free) => `\nZusantara AI uses ${name} as the default provider: ${free}, no API key.`,
  free: "free",
  ...
  omnirouteOffer: "npx zusantara will offer to run it in the background.",
  ...
  nextSteps: "\nNext steps:\n",
  omnirouteHint: "# free AI (only once); or choose another provider: npx zusantara ai:setup",
  zusantaraHint: "# chat with Zusantara AI + dev server in the background",
  serverOnly: (cmd) => `\nOr just run the server: ${cmd} dev`,
  globalTip: 'Tip: npm install -g zusantara so you can just type "zusantara" from any folder.\n',
};
