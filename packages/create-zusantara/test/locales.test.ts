...
describe("template dua bahasa", () => {
  ...
  it("katalog pesan create-zusantara lengkap di kedua bahasa", () => {
  ...
  describe("scaffold --lang en", () => {
    let tmp: string;
    before(() => {
      tmp = fs.mkdtempSync(path.join(os.tmpdir(), "create-zusantara-en-"));
    });
    ...
    it("api: teks, README, dan locale Bahasa Inggris; struktur file sama", () => {
      ...
      assert.match(read("en-app", "zusantara.config.mjs"), /locale: "en"/);
      assert.match(read("id-app", "zusantara.config.mjs"), /locale: "id"/);
