...
import { enqueue, fakeVerify, hashPassword, needsRehash, requireAuth, verifyPassword } from "zusantara";
...
export async function registerUser(input: { name: string; email: string; password: string }): Promise<User | undefined> {
  ...
  const [user] = await db
    ...
    .returning();
  // Send the welcome email in the background (src/app/jobs/welcome-email.ts), so sign-up does not wait for SMTP.
