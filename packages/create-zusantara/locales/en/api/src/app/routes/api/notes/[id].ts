...
import { HttpError, validate, type ZenContext } from "zusantara";
...
// PUT /api/notes/1 (partial updates allowed)
export const PUT = validate({ body: NoteFields.partial() }, async (ctx, { body }) => {
  const note = await findOr404(ctx);
  if (Object.keys(body).length === 0) throw new HttpError(400, "No fields to change");
