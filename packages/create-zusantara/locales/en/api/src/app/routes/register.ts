  ...
  return page(
    ...
    h(
      AuthCard,
      {
        ...
        subtitle: "Fill in the three fields below and you are signed in right away.",
        appName: APP_NAME,
        aside: { title: "Get started in a minute.", text: "New accounts go straight to the dashboard and can write notes. Admins can also manage users." },
        footer: ["Already have an account? ", h("a", { href: "/login" }, "Sign in")],
      },
      h(
        Form,
        { action: "/register" },
        message ? h(Alert, { tone: "error" }, message) : null,
        h(Field, { name: "name", label: "Name", value: values.name, error: errors.name, autocomplete: "name", required: true, autofocus: true }),
        h(Field, { name: "email", label: "Email", type: "email", value: values.email, error: errors.email, autocomplete: "email", required: true }),
        h(Field, { name: "password", label: "Password", type: "password", error: errors.password, hint: "At least 8 characters", autocomplete: "new-password", required: true }),
        h(Button, { block: true, loading: "Creating account…" }, "Create account"),
      ),
    ),
  );
}
...
export const POST = withMiddleware([rateLimit({ windowMs: 15 * 60_000, max: 10 })], async (ctx) => {
  ...
  if (!user) return html(view({ values, errors: { email: "Email is already registered" } }), { status: 409 });
