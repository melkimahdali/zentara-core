...
describe("pemeriksaan tampilan (layoutIssues)", () => {
  ...
  it("teks temuan tersedia dalam Bahasa Indonesia", () => {
    ...
    try {
      ...
      assert.deepEqual(layoutIssues(page).map((i) => i.message), ["Gambar gagal dimuat: img", "Halaman tidak memakai kit UI (page() dari zusantara/ui)"]);
    } finally {
      setLocale("en");
    }
  });
...
describe("pemeriksaan tampilan versi teks (tanpa browser)", () => {
  ...
  const KIT = `<!doctype html><html><head><meta name="viewport" content="width=device-width"><link rel="stylesheet" href="/_zusantara/ui.css?v=1"></head><body class="zu"><h1>Rapi</h1><img src="/logo.png" alt="logo"></body></html>`;
  ...
  it("fakta HTML: kit UI, meta viewport, style, stylesheet, gambar lokal, halaman bawaan", () => {
    ...
    assert.equal(htmlFacts(`<script>window.ZusantaraChat=1</script><style></style>`).framework, true);
    assert.deepEqual(staticIssues(htmlFacts(`<script>window.ZusantaraChat=1</script><style></style>`)), []);
  });
...
describe("journal hasil tugas AI", () => {
  ...
  it("entri berisi ringkasan saja, dibaca kembali oleh zusantara ai:log", async () => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-tasklog-"));
  ...
  it("journal kosong", async () => {
    ...
    const out: string[] = [];
    assert.equal(await run(["ai:log"], { cwd: dir, out: (l) => out.push(l), err: () => {} }), 0);
    assert.match(out.join("\n"), /Belum ada tugas Zusantara AI/);
  });
});

describe("devtools: zusantara view dari terminal lain", () => {
  it("port dan token di .zusantara/devtools.json (hanya pemilik), dihapus saat berhenti; /view memakai viewPage", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-dtinfo-"));
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${dt.port}/view`, {
        method: "POST",
        headers: { "X-Zusantara-Token": dt.token, "Content-Type": "application/json" },
