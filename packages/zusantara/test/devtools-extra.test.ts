...
describe("jejak request (devtrace)", () => {
  ...
  it("insert berulang bukan N+1; aset /_zusantara/ tidak disimpan", () => {
    assert.deepEqual(repeatedQueries([1, 2, 3].map(() => ({ sql: "insert into a values (?)" }))), []);
    const before = runWithTrace({ method: "GET", path: "/_zusantara/ui.css" }, (tr) => tr)!;
    finishTrace(before, { status: 200 });
    assert.deepEqual(sessionPreview({ password: "x", nama: "Ani", data: { a: 1 } }), { password: "•••", nama: "Ani", data: '{"a":1}' });
  });
});

describe("runtime saat zusantara dev: jejak, varian, sumber elemen", () => {
  let dir: string;
  let restore: () => void = () => {};
  ...
  before(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-devtrace-"));
    fs.mkdirSync(path.join(dir, "routes"));
    // Seperti proyek Zusantara: ESM, supaya route memakai modul framework yang sama dengan runtime.
...
`,
  ...
  it("header id request, probe data-request, endpoint jejak dengan token, N+1 dan log", async () => {
    ...
    try {
      ...
      const id = res.headers.get("x-zusantara-request");
      assert.ok(id);
      assert.match(html, new RegExp(`data-request="${id}"`));
      assert.equal((await fetch(`${base}/_zusantara/dev/requests`)).status, 401);
      assert.equal((await fetch(`${base}/_zusantara/dev/requests`, { headers: { "X-Zusantara-Token": "salah" } })).status, 401);
      const trace = (await (await fetch(`${base}/_zusantara/dev/requests/${id}`, { headers: { "X-Zusantara-Token": "tok" } })).json()) as RequestTrace;
      assert.equal(trace.status, 200);
      ...
      const list = (await (await fetch(`${base}/_zusantara/dev/requests`, { headers: { "X-Zusantara-Token": "tok" } })).json()) as { requests: RequestTrace[] };
      assert.equal(list.requests[0]!.id, id);
      assert.equal((await fetch(`${base}/_zusantara/dev/requests/tidak-ada`, { headers: { "X-Zusantara-Token": "tok" } })).status, 404);
    } finally {
      await runtime.stop();
    }
  });
  ...
  it("varian bahasa dan mode per request, parameter dibuang sebelum routing; tangkapan layar tanpa widget", async () => {
    ...
    const { runtime, base } = await start(true);
    try {
      const html = await (await fetch(`${base}/?a=1&__zusantara_lang=en&__zusantara_mode=dark`)).text();
      assert.match(html, /<html lang="en" data-zu-mode="dark"/);
      assert.match(html, /\?a=1</);
      assert.doesNotMatch(html, /__zusantara_/);
      // Request berikutnya kembali ke bahasa dan mode proses.
      assert.match(await (await fetch(`${base}/`)).text(), /<html lang="id">/);
      assert.deepEqual(await (await fetch(`${base}/data?__zusantara_mode=dark`)).json(), { search: "" });
      const shot = await (await fetch(`${base}/?__zusantara_shot=1`)).text();
      assert.doesNotMatch(shot, /_zusantara\/dev\//);
    } finally {
      await runtime.stop();
    }
  });
  ...
  it("produksi: tanpa jejak, tanpa varian, tanpa data-zsrc", async () => {
    ...
    try {
      const res = await fetch(`${base}/?__zusantara_lang=en`);
      const html = await res.text();
      assert.equal(res.headers.get("x-zusantara-request"), null);
      assert.doesNotMatch(html, /data-zsrc|lang="en"/);
      assert.equal((await fetch(`${base}/_zusantara/dev/requests`, { headers: { "X-Zusantara-Token": "tok" } })).status, 404);
    } finally {
      await runtime.stop();
    }
  });
...
describe("view_page: skor, varian, rekaman langkah, jejak", () => {
  ...
  it("varian, ukuran tablet, dan parameter URL", () => {
    ...
    assert.equal(withVariant("/produk?x=1#a", { theme: "dark", lang: "en" }), "/produk?x=1&__zusantara_mode=dark&__zusantara_lang=en#a");
    assert.equal(withVariant("/", { shot: true }), "/?__zusantara_shot=1");
  });
  ...
  it("view_page meneruskan varian ke tab, menampilkan jejak request; request_log", async () => {
    ...
    assert.deepEqual(seen, ["/produk?__zusantara_mode=dark&__zusantara_lang=en"]);
    ...
    await assert.rejects(tool("request_log").run({}, { ...ctx, viewer: undefined }), /only available while the app runs under `zusantara dev`/);
  });
...
describe("muat ulang otomatis dan zusantara requests", () => {
  let root: string;
  before(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-reload-"));
  });
  ...
  it("server aplikasi mulai ulang -> tab yang terhubung menerima perintah reload", async () => {
    ...
    const headers = { "X-Zusantara-Token": dt.token, "Content-Type": "application/json" };
    try {
  ...
  it("zusantara requests tanpa server dev: pesan jelas, kode keluar 1", async () => {
    const out: string[] = [];
    const err: string[] = [];
