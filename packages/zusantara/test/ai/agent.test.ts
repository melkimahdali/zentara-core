...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-ai-"));
...
describe("keamanan tool", () => {
  ...
  it("tool zusantara: routes & jobs langsung jalan; make:job minta persetujuan dan bisa di-undo", async () => {
    ...
    const run = (input: Record<string, unknown>) => agentTools.find((t) => t.spec.name === "zusantara")!.run(input, ctx);
    assert.match(await run({ command: "routes" }), /^BERHASIL: zusantara routes\n[\s\S]*\/halo/);
    assert.equal(asked.length, 0, "routes baca-saja, tanpa persetujuan");

    assert.match(await run({ command: "make:job", args: ["laporan-harian", "--schedule", "0 7 * * *"] }), /^BERHASIL: zusantara make:job laporan-harian/);
...
describe("persetujuan", () => {
  ...
  it("mode auto: perubahan biasa langsung; krusial (hapus, config, install) tetap ditanyakan", async () => {
    ...
    await assert.rejects(write(ctx, "zusantara.config.mjs"), /tidak menyetujui/);
...
describe("resolveAiConfig", () => {
  ...
  it("env dan validasi", () => {
    assert.equal(resolveAiConfig(undefined, { ZUSANTARA_AI_MODE: "auto" }).mode, "auto");
