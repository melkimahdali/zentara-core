...
let root: string;
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-sessions-"));
...
describe("penyimpanan sesi", () => {
  it("simpan, muat, urutkan terbaru dulu, dan hanya bisa dibaca pemilik", () => {
    ...
    if (process.platform !== "win32") assert.equal(fs.statSync(path.join(root, ".zusantara", "sessions", "a.json")).mode & 0o777, 0o600);
  });
  ...
  it("ID dengan path ditolak; file rusak dilewati; sesi lama dipangkas", () => {
    ...
    fs.mkdirSync(path.join(root, ".zusantara", "sessions"), { recursive: true });
    fs.writeFileSync(path.join(root, ".zusantara", "sessions", "rusak.json"), "{");
    fs.utimesSync(path.join(root, ".zusantara", "sessions", "rusak.json"), new Date(2000, 0, 1), new Date(2000, 0, 1));
    const dir = path.join(root, ".zusantara", "sessions");
    for (let i = 0; i < 35; i++) {
