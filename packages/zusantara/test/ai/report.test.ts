...
describe("zusantara ai --report", () => {
  ...
  before(async () => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "zusantara-report-"));
    ...
    env("ZUSANTARA_AI_ORDER", "openai");
    env("ZUSANTARA_AI_MODE", undefined);
  });
  after(() => {
  ...
  it("menulis hasil tugas sebagai JSON ke file yang diminta", async () => {
    ...
    assert.equal(typeof report.zusantara, "string");
  ...
  it("tanpa nama file ditulis ke .zusantara/ai-report.json", async () => {
    const res = await cli(["ai", "jelaskan proyek ini", "--report"]);
    assert.equal(res.code, 0, res.err);
    const report = JSON.parse(fs.readFileSync(path.join(dir, ".zusantara", "ai-report.json"), "utf8")) as { mode: string; status: string };
