...
describe("halaman error saat pengembangan (debug)", () => {
  ...
  it("500: pesan, lokasi file, potongan kode, dan header rahasia disembunyikan", async () => {
    ...
    // Tanpa `zusantara dev` tidak ada chat AI.
    assert.doesNotMatch(body, /window\.ZusantaraChat =/);
  });
  ...
  it("404: daftar route dan saran perintah", async () => {
    ...
    assert.match(body, /npx zusantara make:route belum\/ada/);
  });
...
describe("renderErrorPage", () => {
  ...
  it("menyertakan chat hanya bila devtools tersedia", () => {
    process.env.ZUSANTARA_DEVTOOLS_PORT = "4567";
    process.env.ZUSANTARA_DEVTOOLS_TOKEN = "tok";
    process.env.ZUSANTARA_DEV = "1";
    try {
      const body = renderErrorPage(new Error("x"), req);
      assert.match(body, /Tanya Zusantara AI/);
      assert.match(body, /"devtools":\{"port":4567,"token":"tok"\}/);
    } finally {
      delete process.env.ZUSANTARA_DEVTOOLS_PORT;
      delete process.env.ZUSANTARA_DEVTOOLS_TOKEN;
      delete process.env.ZUSANTARA_DEV;
    }
  });
});
...
describe("parseStack & highlight", () => {
  it("membedakan kode aplikasi dari node_modules dan internal", () => {
    const stack = [
      ...
      "    at async compose (file:///proj/node_modules/zusantara/dist/core/middleware.js:10:5)",
...
describe("welcomePage", () => {
  it("produksi: tanpa daftar route dan tanpa chat", () => {
    ...
    assert.doesNotMatch(body, /\/rahasia|window\.ZusantaraChat =/);
  });

  it("pengembangan lewat zusantara dev: daftar route dan chat AI", () => {
    setAppInfo({ appName: "Zusantara App", env: "development", debug: true, root: process.cwd(), routes: [{ pattern: "/api/produk", methods: ["GET", "POST"], file: "x.ts" }] });
    process.env.ZUSANTARA_DEVTOOLS_PORT = "4567";
    process.env.ZUSANTARA_DEVTOOLS_TOKEN = "tok";
    process.env.ZUSANTARA_DEV = "1";
    try {
      ...
      assert.match(body, /window\.ZusantaraChat =/);
