...
describe("tampilan Ink (CLI interaktif)", () => {
  it("header, input, dan baris status tampil", async () => {
    ...
    assert.match(frame, /Zusantara Core v9\.9\.9/);
  ...
  it("tata letak biasa: animasi logo pembuka, lalu header masuk ke riwayat", async () => {
    ...
    await tick(60);
    assert.doesNotMatch(strip(ui.lastFrame()), /Zusantara Core v9/, "info belum tampil di awal animasi");
    ...
    assert.equal(frame.match(/Zusantara Core v9/g)?.length, 1, "header tidak tergambar dua kali");
    ui.unmount();
  });
  ...
  it("layar penuh: header terkunci di atas, log bisa digulir, input di bawah", async () => {
    ...
    for (const re of [/Zusantara Core v9\.9\.9/, /Rooted here\. Built for what's next\./, /OmniRoute \(gratis\) · minta persetujuan/, /~\/proyek/, /● http:\/\/localhost:3000/]) assert.match(header, re);
    ...
    lines = frame.split("\n");
    assert.match(lines.slice(0, 8).join("\n"), /Zusantara Core/, "header tetap di tempatnya saat menggulir");
  ...
  it("layar penuh di terminal pendek: header ringkas dua baris dalam bingkai", async () => {
    ...
    assert.match(lines[1]!, /◆ Zusantara Core v9\.9\.9 {2}OmniRoute \(gratis\) · minta persetujuan +● http:\/\/localhost:3000/);
...
describe("tata letak layar penuh (fungsi murni)", () => {
  ...
  it("layar penuh hanya untuk terminal interaktif yang cukup tinggi", () => {
    ...
    assert.equal(fullscreenEnabled(undefined, { isTTY: true, rows: 40 }, { ZUSANTARA_FULLSCREEN: "off" }), false);
    assert.equal(fullscreenEnabled(false, { isTTY: true, rows: 40 }, { ZUSANTARA_FULLSCREEN: "on" }), true);
  ...
  it("header dengan logo hanya di terminal yang cukup besar; layar alternatif & judul tab", () => {
    ...
    assert.equal(setTitle("zusantara\u0007\u001b"), "\u001b]0;zusantara\u0007", "karakter kontrol dibuang");
