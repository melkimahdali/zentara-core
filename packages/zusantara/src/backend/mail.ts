 ...
 *   - log     (bawaan saat pengembangan: email dicetak ke log dan disimpan di .zusantara/mail/*.eml)
 ...
 * (wajib untuk SMTP; untuk log/memory bawaannya "Zusantara <noreply@localhost>").
...
export interface MailConfig {
  ...
  from?: string;
  logger?: ZenLogger;
  /** Folder untuk email transport "log". Default: .zusantara/mail. */
  logDir?: string;
}
...
export function buildMessage(message: MailMessage, from: string, id: string): string {
  ...
  const boundary = () => `zusantara-${randomBytes(12).toString("hex")}`;
...
export async function sendMail(message: MailMessage): Promise<SentMail> {
  ...
  const from = message.from ?? config.from ?? process.env.MAIL_FROM ?? (url === "log" || url === "memory" ? "Zusantara <noreply@localhost>" : undefined);
  ...
  if (url === "log") {
    const dir = path.resolve(config.logDir ?? path.join(".zusantara", "mail"));
    let file = "";
    try {
