...
/** Tabel antrean dari versi sebelum ganti nama (zentara_*) dipakai terus dengan nama baru. */
function renameLegacyTables(db: DatabaseSync): void {
  const exists = (name: string) => db.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ?").get(name) !== undefined;
  for (const [from, to] of [["zentara_jobs", "zusantara_jobs"], ["zentara_schedules", "zusantara_schedules"]] as const) {
    try {
      if (exists(from) && !exists(to)) db.exec(`ALTER TABLE ${from} RENAME TO ${to}`);
    } catch {
      // Proses lain baru saja mengganti namanya.
    }
  }
  if (exists("zusantara_jobs")) db.exec("DROP INDEX IF EXISTS zentara_jobs_due");
}
...
export class SqliteJobStore implements JobStore {
  ...
  constructor(file: string) {
    ...
    renameLegacyTables(this.db);
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS zusantara_jobs (
        id TEXT PRIMARY KEY,
        name TEXT NOT NULL,
        data TEXT NOT NULL,
      ...
      CREATE INDEX IF NOT EXISTS zusantara_jobs_due ON zusantara_jobs (status, run_at);
      CREATE TABLE IF NOT EXISTS zusantara_schedules (name TEXT PRIMARY KEY, last_slot INTEGER NOT NULL);
    `);
  ...
  add(j: JobRecord): void {
    this.db
      .prepare("INSERT INTO zusantara_jobs (id, name, data, status, attempts, max_attempts, run_at, locked_until, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)")
  ...
  claim(now: number, lockMs: number): JobRecord | undefined {
    ...
    const row = this.db
      .prepare(
        `UPDATE zusantara_jobs SET status = 'running', locked_until = ?, attempts = attempts + 1
         WHERE id = (SELECT id FROM zusantara_jobs
  ...
  complete(id: string, now: number): void {
    this.db.prepare("UPDATE zusantara_jobs SET status = 'done', finished_at = ?, locked_until = 0 WHERE id = ?").run(now, id);
  }

  retry(id: string, runAt: number, error: string): void {
    this.db.prepare("UPDATE zusantara_jobs SET status = 'queued', run_at = ?, last_error = ?, locked_until = 0 WHERE id = ?").run(runAt, error, id);
  }

  fail(id: string, error: string, now: number): void {
    this.db.prepare("UPDATE zusantara_jobs SET status = 'failed', last_error = ?, finished_at = ?, locked_until = 0 WHERE id = ?").run(error, now, id);
  }

  claimSchedule(name: string, slot: number): boolean {
    const result = this.db
      .prepare("INSERT INTO zusantara_schedules (name, last_slot) VALUES (?, ?) ON CONFLICT(name) DO UPDATE SET last_slot = excluded.last_slot WHERE last_slot < excluded.last_slot")
      .run(name, slot);
  ...
  counts(): Record<JobStatus, number> {
    const counts: Record<JobStatus, number> = { queued: 0, running: 0, done: 0, failed: 0 };
    for (const r of this.db.prepare("SELECT status, COUNT(*) AS n FROM zusantara_jobs GROUP BY status").all() as { status: JobStatus; n: number }[]) {
  ...
  list(options: { status?: JobStatus; limit?: number } = {}): JobRecord[] {
    ...
    const rows = options.status
      ? this.db.prepare("SELECT * FROM zusantara_jobs WHERE status = ? ORDER BY created_at DESC LIMIT ?").all(options.status, limit)
      : this.db.prepare("SELECT * FROM zusantara_jobs ORDER BY created_at DESC LIMIT ?").all(limit);
    return (rows as Record<string, unknown>[]).map((r) => SqliteJobStore.row(r));
  }

  prune(before: number): number {
    return Number(this.db.prepare("DELETE FROM zusantara_jobs WHERE status = 'done' AND finished_at < ?").run(before).changes);
  }
...
export class JobQueue {
  ...
  /** Jalankan job langsung di proses ini tanpa antrean (dipakai `zusantara jobs:run`). */
