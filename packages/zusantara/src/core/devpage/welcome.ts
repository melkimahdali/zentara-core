...
import { LOGO_SVG, renderPage, WORDMARK, ZUSANTARA_VERSION } from "./theme.js";
...
(function(){
  ...
  if (box && data.devtools) ZusantaraChat.mount(box, { port: data.devtools.port, token: data.devtools.token, storageKey: "welcome", emptyText: data.emptyText, suggestions: data.suggestions, placeholder: data.placeholder, t: data.chat });
})();
`;
...
/**
 * Halaman sambutan bawaan Zusantara. Pakai sebagai route awal:
 *
 *   // src/app/routes/index.ts
 *   export { welcomePage as GET } from "zusantara";
 *
 * Saat pengembangan (lewat `zusantara dev`) halaman ini memuat chat Zusantara AI dan daftar route.
...
export function welcomePage(): string {
  ...
  const chat = devtools
    ? `<section class="zx-card zw-chat"><div class="zw-chat-head">${LOGO_SVG}<strong>Zusantara AI</strong><span class="zx-muted" style="font-size:13px">${escapeHtml(m.assistant)}</span></div><div class="zw-chat-body"></div></section>`
...
<p class="zx-muted" style="margin:0 0 14px">${escapeHtml(m.buildLead)} ${escapeHtml(debug ? m.runThroughZusantara : m.startFromTerminal)}</p>
<div class="zw-cmds">${cmd("npx zusantara")}${cmd("npx zusantara ai:setup")}${debug ? cmd("npx zusantara dev") : ""}</div></section>`;
...
<header class="zx-top">${LOGO_SVG}<div class="zx-brand">${WORDMARK}<small>${escapeHtml(info.appName)}</small></div><div class="zx-spacer"></div>${debug ? `<span class="zx-badge"><span class="dot"></span>${escapeHtml(info.env)}</span>` : ""}<span class="zx-badge off"><span class="dot"></span>v${escapeHtml(ZUSANTARA_VERSION)}</span></header>
...
<section class="zx-card zx-pad"><h2>${escapeHtml(m.commands)}</h2><div class="zw-cmds"><p>${escapeHtml(m.cmdChat)}</p>${cmd("npx zusantara")}<p>${escapeHtml(m.cmdSetup)}</p>${cmd("npx zusantara ai:setup")}<p>${escapeHtml(m.cmdUndo)}</p>${cmd("npx zusantara undo")}</div></section>
...
<a href="https://www.npmjs.com/package/zusantara" target="_blank" rel="noopener">${escapeHtml(m.npm)} <span>zusantara →</span></a>
<a href="https://github.com/melkimahdali/zusantara-core/issues" target="_blank" rel="noopener">${escapeHtml(m.issues)} <span>GitHub →</span></a></div></section></div>
<p class="zx-foot">${m.footHtml} · Zusantara Core — Rooted here. Built for what&#39;s next.</p></div>`;

  return renderPage({
    title: info.appName === "Zusantara App" ? "Zusantara" : info.appName,
    body,
