...
import { highlight, LOGO_SVG, renderPage, WORDMARK, ZUSANTARA_VERSION } from "./theme.js";
...
const SECRET_HEADERS = new Set(["cookie", "authorization", "proxy-authorization", "x-zusantara-token", "x-api-key"]);
...
(function(){
  ...
  function open(first){
    ...
    if (!chat) chat = ZusantaraChat.mount(document.querySelector(".ze-drawer-body"), { port: data.devtools.port, token: data.devtools.token, storageKey: "error", context: data.context, placeholder: data.placeholder, t: data.chat });
...
function drawer(): string {
  return `<aside class="ze-drawer" aria-label="Zusantara AI"><div class="ze-drawer-head">${LOGO_SVG}<strong>Zusantara AI</strong><div class="zx-spacer"></div><button class="zx-btn small" data-close-drawer>${escapeHtml(t().dev.error.close)}</button></div><div class="ze-drawer-body"></div></aside>`;
}

/** Ringkasan error dalam teks biasa: untuk tombol salin dan konteks Zusantara AI. */
...
/** Halaman error lengkap untuk mode debug: pesan, stack trace dengan kode, detail request, dan Zusantara AI. */
...
<section class="zx-card zx-pad ze-sec"><h2>${escapeHtml(m.environment)}</h2><dl class="zx-kv"><dt>Zusantara</dt><dd>${escapeHtml(ZUSANTARA_VERSION)}</dd><dt>Node.js</dt><dd>${escapeHtml(process.version)}</dd><dt>Env</dt><dd>${escapeHtml(info.env)}</dd><dt>${escapeHtml(m.folder)}</dt><dd>${escapeHtml(info.root)}</dd></dl></section>
...
<section class="zx-card zx-pad"><h2>${escapeHtml(m.createManually)}</h2><p class="zx-muted" style="margin-top:0">${m.createManuallyHtml}</p><div class="zx-cmd"><span>npx zusantara make:route ${escapeHtml(suggestion)}</span></div></section></div>
