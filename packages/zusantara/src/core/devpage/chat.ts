/**
 * Chat Zusantara AI untuk browser (halaman sambutan, halaman error, dan widget mengambang saat pengembangan).
 * Berbicara dengan server devtools milik `zusantara dev` di 127.0.0.1 memakai token sesi.
...
(function(){
  ...
  function mount(root, opts){
    ...
    var key = "zusantara-chat:" + opts.port + ":" + (opts.storageKey || "main");
    ...
    // Input suara (Web Speech API) dalam bahasa Zusantara; tombolnya tidak muncul bila browser tidak mendukung.
    ...
    function api(path, body){
      return fetch(base + path, { method: "POST", headers: { "Content-Type": "application/json", "X-Zusantara-Token": opts.token }, body: JSON.stringify(body || {}) });
    }
    ...
    fetch(base + "/status", { headers: { "X-Zusantara-Token": opts.token } }).then(function(r){ return r.json(); }).then(function(s){
    ...
    function ask(message, context){
      ...
      fetch(base + "/chat", { method: "POST", headers: { "Content-Type": "application/json", "X-Zusantara-Token": opts.token }, body: JSON.stringify({ message: message, context: context || opts.context || "", page: page || undefined, pageId: opts.pageId ? opts.pageId() : undefined }), signal: controller.signal })
        .then(function(res){
  ...
  window.ZusantaraChat = { mount: mount };
})();
`;
