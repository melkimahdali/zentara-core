...
export interface HostUI {
  ...
  clear?(): void;
  /**
   * Serahkan terminal ke proses lain (mis. `npm create zusantara`, `npm install -g omniroute`) lalu kembali.
...
export interface HostOptions {
  ...
  /** Bahasa belum pernah dipilih (tanpa ZUSANTARA_LANG, `locale` di config, atau `zusantara lang`): tanyakan saat dibuka. */
  askLanguage?: boolean;
}
...
export async function createReplHost(options: HostOptions, ui: HostUI): Promise<ReplHost> {
  ...
  /**
   * Buat proyek baru tanpa keluar dari tampilan Zusantara: nama folder dan template ditanyakan di sini,
   * lalu create-zusantara berjalan di latar belakang (tanpa pertanyaan) dengan progres di spinner.
   * Setelah selesai, Zusantara dibuka di folder proyek baru. undefined = batal, lanjut di folder ini.
   */
  ...
  /** Proses create-zusantara yang sedang berjalan (Esc membatalkannya). */
  ...
  async function createProject(): Promise<number | undefined> {
    ...
    const m = t().host;
    const answer = await ui.ask(m.projectFolder, { placeholder: "zusantara-app" });
    if (answer === undefined) return undefined;
    const name = projectSlug(answer) || "zusantara-app";
    ...
    const code = await new Promise<number>((resolve) => {
      // npx --yes: tanpa pertanyaan "Ok to proceed?"; create-zusantara --yes: tanpa pertanyaan lanjutan.
      // ZUSANTARA_CREATE_PACKAGE / ZUSANTARA_CREATE_ARGS: untuk uji e2e (tarball lokal, --zusantara-spec file:...).
      const pkg = process.env.ZUSANTARA_CREATE_PACKAGE || "create-zusantara@latest";
      const extra = (process.env.ZUSANTARA_CREATE_ARGS ?? "").split(/\s+/).filter(Boolean);
      const cmd = platformCommand("npx", ["--yes", `--package=${pkg}`, "--", "create-zusantara", name, "--template", template, "--lang", lang, "--yes", ...extra]);
    ...
    // Zusantara di folder proyek baru mengambil alih terminal; saat ditutup, CLI ini ikut selesai.
    return ui.suspend(
      () =>
    ...
    switch (cmd) {
      ...
      case "language": {
        ...
        try {
          ...
          // tidak bisa menulis ~/.zusantara: bahasa tetap berlaku untuk sesi ini
        }
