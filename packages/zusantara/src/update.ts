...
export interface UpdateCheckOptions {
  current: string;
  /** Default ~/.zusantara/update-check.json */
  cacheFile?: string;
  now?: number;
...
/**
 * Cek versi terbaru `zusantara` langsung ke registry npm (tanpa cache npm lokal yang bisa tertinggal),
 * paling sering sekali sehari. Mengembalikan versi baru bila ada. Tidak pernah melempar error.
 * Matikan dengan ZUSANTARA_NO_UPDATE_CHECK=1 (otomatis mati di CI).
 */
export async function checkForUpdate(options: UpdateCheckOptions): Promise<string | undefined> {
  const env = options.env ?? process.env;
  if (env.ZUSANTARA_NO_UPDATE_CHECK || env.CI || env.NO_UPDATE_NOTIFIER) return undefined;
  const cacheFile = options.cacheFile ?? path.join(os.homedir(), ".zusantara", "update-check.json");
  ...
  try {
    const res = await (options.fetchImpl ?? fetch)("https://registry.npmjs.org/zusantara/latest", {
