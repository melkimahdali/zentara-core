...
/** Token warna Zusantara Core (Brand Identity Guidelines, Concept C · Nusantara Tech). */
export const BRAND = {
  teal: "#2ED3B7",
  gold: "#C89B52",
  obsidian: "#0D1719",
...
export const DOCS_URL = "https://zusantara.morixa.id/";
...
/**
 * Banner Zusantara Core: logo di kiri dan teks di kanan pada terminal lebar, teks saja pada terminal
...
export function banner(options: BannerOptions): string[] {
  ...
  const text = [
    `${bold("Zusantara")} ${bold(teal("Core"))}  ${slate(`v${options.version}`)}`,
    slate(DESCRIPTION),
    gold(TAGLINE),
    ...(options.details?.length ? ["", ...options.details] : []),
  ];

  if (columns < 44) return [`${teal("Z>")} ${bold("Zusantara")} ${teal("Core")} ${slate(`v${options.version}`)}`];
