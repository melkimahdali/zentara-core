...
/**
 * Pengetahuan tentang tata letak (layout) proyek untuk Zusantara AI, agar halaman baru memakai layout
 * yang sudah ada (appPage di src/app/lib/ui.ts atau page() dari zusantara/ui), bukan membuat HTML dan
 * CSS sendiri.
 */
...
export function layoutSnapshot(root: string): string[] {
  ...
  if (layout) {
    const uses = layout.exports.includes("appPage")
      ? `Every signed-in page MUST use appPage(ctx, { title, active }, ...children) from ${layout.file}; public pages use page() from "zusantara/ui". Add new pages to the navigation menu in navFor() in ${layout.file}.`
    ...
    lines.push(`App layout: none yet. Build full pages with page() and the components from "zusantara/ui".`);
  }
...
export function layoutWarning(root: string, rel: string, content: string): string | undefined {
  ...
  const use = layout?.exports.includes("appPage")
    ? `appPage(ctx, { title, active }, ...children) from ${layout.file} (signed-in pages) or page() from "zusantara/ui" (public pages)`
    : `page({ title }, ...body) and the components from "zusantara/ui"`;
