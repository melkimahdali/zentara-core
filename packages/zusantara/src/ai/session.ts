...
export interface AiSessionOptions {
  ...
  /** Simpan percakapan ke .zusantara/sessions setelah setiap permintaan (untuk /resume). */
  persist?: boolean;
...
export interface AiSession {
  ...
  /** ID sesi saat ini (nama file di .zusantara/sessions). */
  readonly id: string;
...
export function createAiSession(options: AiSessionOptions): AiSession {
  ...
  function persist(): void {
    ...
    try {
      // Gambar (tangkapan layar) tidak disimpan: file PNG-nya sudah ada di .zusantara/screenshots/.
  ...
  return {
    ...
    async run(task, runOptions = {}) {
      ...
      first = false;
      title ||= task.split("\n")[0]!.slice(0, 100);
      // Satu jurnal per perintah, jadi `zusantara undo` membatalkan perintah terakhir saja.
...
/** Sesi AI untuk satu perintah di terminal (`zusantara "..."`), dengan ringkasan di akhir. */
