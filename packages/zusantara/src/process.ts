...
/**
 * CLI zusantara milik proyek (`<proyek>/node_modules/zusantara/dist/cli.js`), bila proyek terdekat dari
 * `root` (folder dengan package.json) memakai zusantara dan CLI itu berbeda dari yang sedang berjalan
 * (`self`). Perintah yang butuh dependency proyek, seperti `db:*` yang memakai drizzle-orm, dijalankan
 * lewat CLI ini: CLI global (`npm install -g zusantara`) tidak punya dependency proyek.
 */
export function findLocalCli(root: string, self?: string): string | undefined {
  ...
  try {
    ...
    if (!pkg.dependencies?.zusantara && !pkg.devDependencies?.zusantara) return undefined;
  } catch {
    return undefined;
  }
  const candidate = path.join(dir, "node_modules", "zusantara", "dist", "cli.js");
